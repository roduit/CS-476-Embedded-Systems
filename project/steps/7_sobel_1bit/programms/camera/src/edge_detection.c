...
void boosted_compare(uint32_t *new_image, uint32_t *old_image, uint8_t *grayscale, uint16_t *result, int size) {
    ...
    for (int i = 0; i < (int)(size / 256); i += 1) {
        ...
        // New image
        DMA_setupAddr((uint32_t)&new_image[0] + i*32, newImgStartAddrCI);
        DMA_startTransferBlocking(1);

        // // Old image
        DMA_setupAddr((uint32_t)&old_image[0] + i*32, oldImgStartAddrCI);
        DMA_startTransferBlocking(1);
        ...
        for (int j = 0; j < 256; j+=2) {
            if (j % 32 == 0) {
                ...
                mask = (swap_u32(tmp_new) ^ swap_u32(tmp_old)) & swap_u32(tmp_new);
                // mask = (new_image[sob_idx] ^ old_image[sob_idx]) & new_image[sob_idx];
                sob_idx++;
            }
...
void compute_sobel_v1(uint32_t grayscaleAddr, volatile uint32_t * sobelImage, uint32_t cameraWidth, uint32_t cameraHeight, uint8_t threshold) {
    ...
    for (line_index; line_index < effectiveHeight; line_index++) {
        ...
        for (col_index = 0; col_index < effectiveWidth; col_index++) {
            ...
            if ((col_index + 1) % 8 == 0) {
                tmp_result =   (tmp_sobel_result & 0xF) << 28 | 
                ...
                DMA_writeCIMem(startSobelBufferAddr + idx, swap_u32(tmp_result));
                idx++;
            }
        ...
        DMA_setupAddr((uint32_t)& sobelImage[0] + (line_index + 1)*(effectiveWidth / 2) + 1, startSobelBufferAddr);
        DMA_startTransferBlocking(2);
    }
}