...
int main() {
  ...
  for (ramAddress = usedCiRamAddress; ramAddress < usedCiRamAddress+usedBlocksize; ramAddress++) {
    asm volatile("l.nios_rrr %[out1],%[in1],r0,20" :[out1]"=r"(ramData):[in1] "r"(ramAddress&0xFFF)); // we control that the memory is correct
    if (ramData != 0) printf("Error at address %0d : %0d\n", ramAddress, ramData);
  }
  printf("Writing ci memory\n");
  for (ramAddress = 0; ramAddress < 512; ramAddress++) {
    asm volatile("l.nios_rrr r0,%[in1],%[in2],20" ::[in1] "r"(ramAddress | writeBit), [in2]"r"(100)); // we fill the memory
  }
  ...
  for (ramAddress = 0; ramAddress < usedBlocksize; ramAddress++) {
    ...
    if (ramData != (100)) printf("Error at address %0d : %0d, expected 100\n", ramAddress, ramData);
  }
  printf("Done\n");
}
